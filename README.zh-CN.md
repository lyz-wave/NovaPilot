...
- **四层证据接地防线**:检索接地 → 规则核验 → 语义复核 → **适用范围契约**,任何一层可拒绝推荐;三轮无证据即携完整论证链转专家,绝不硬编。
- **接地 ≠ 相关**:前三层查的都是引用号是否**真实**,没有一层查证据是否回答了**客户问的那件事**。8 条对抗样例第一次跑,**7 条被自信放行**。第四层拿问题原文去比对知识库各文档声明的 `appliesTo` —— 判据来自知识库,不是禁词表。实测分离度:19 条合法问题假阳 0、8 条越界问题漏检 0;端到端漏放 7/8 → **0/8**,并挂上 P0 发版门禁。详见 `docs/B5-…`。
...
- **离线运行是硬不变式**:405 个单测 + 14 个 Playwright 验收脚本全部可离线复现。检索的稠密通道**可确定性降级**
...
- **测试**:Vitest(405 个单测)+ Playwright(14 个 E2E 验收脚本)
